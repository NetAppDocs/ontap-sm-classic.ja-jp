...
---
= System Manager - ONTAP 9.7 以前を使用して、ローカル Windows グループを削除します
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ONTAP System Manager クラシック（ ONTAP 9.7 以前で使用可能）を使用できます。 Storage Virtual Machine （ SVM ）に格納されたデータへのアクセス権の定義やグループメンバーへの SVM のユーザ権限の割り当てに使用する Windows ローカルグループが不要になった場合に Storage Virtual Machine （ SVM ）から削除する。
