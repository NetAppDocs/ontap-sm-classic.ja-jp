...
summary: このコンテンツでは、 ONTAP 9.7 以前の ONTAP 9 リリースで使用できる ONTAP System Manager のクラシックインターフェイスを使用して、新規または既存の Storage Virtual Machine （ SVM ）の新しいボリュームへの SMB / CIFS アクセスを設定する簡単な方法について説明しています。 
---
= SMB / CIFS の設定の概要
:icons: font
:imagesdir: ../media/
...
[role="lead"]
このコンテンツでは、 ONTAP 9.7 以前の ONTAP 9 リリースで使用できる ONTAP System Manager のクラシックインターフェイスを使用して、新規または既存の Storage Virtual Machine （ SVM ）の新しいボリュームへの SMB / CIFS アクセスを設定する簡単な方法について説明しています。
